	...
	class scss_parser {
		...
		protected function parseChunk() {
			...
			if (isset($this->buffer[$this->count]) && $this->buffer[$this->count] == "@") {
				...
				if (
					$this->literal("@mixin") && $this->keyword($mixinName) && ($this->argumentDef($args) || true)
					&& $this->literal("{")
				) {
				...
				if (
					$this->literal("@include") && $this->keyword($mixinName)
					&& ($this->literal("(")
				...
				if (
					$this->literal("@each") && $this->variable($varName) && $this->literal("in") && $this->valueList($list)
					&& $this->literal("{")
				) {
				...
				if (
					$this->literal("@for") && $this->variable($varName) && $this->literal("from") && $this->expression($start)
				...
				if (
					$this->literal("@", false) && $this->keyword($dirName) && ($this->openString("{", $dirValue) || true)
					&& $this->literal("{")
				) {
		...
		protected function mediaExpression(&$out) {
			...
			$value = null;
			if (
				$this->literal("(") && $this->expression($feature) && ($this->literal(":") && $this->expression($value) || true)
				&& $this->literal(")")
			) {
