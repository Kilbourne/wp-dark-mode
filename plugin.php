 ...
 * @since  1.2.2
 */
if (!defined('WPDM_BASE_PATH'))
	define('WPDM_BASE_PATH', plugin_dir_path(__FILE__));
...
if (!defined('WPDM_BASE_URL'))
	define('WPDM_BASE_URL', plugin_dir_url(__FILE__));


if (!file_exists(WPDM_BASE_PATH . 'vendor/autoload.php'))
	return;

require_once WPDM_BASE_PATH . 'vendor/autoload.php';
...
if (!class_exists('WP_Dark_Mode')) {
	...
	final class WP_Dark_Mode {
		...
		public function activation() {
			new WPDM\includes\WP_Dark_Mode_Install;
		...
		public function init() {
			...
			if (did_action('elementor/loaded')) {
				require_once WPDM_BASE_PATH . 'elementor/modules/controls/init.php';
			}
		}
		...
		public function load_files() {
			...
			//core includes
			require_once WPDM_BASE_PATH . 'includes/functions.php';

			new WPDM\includes\WP_Dark_Mode_Enqueue;
			new WPDM\includes\WP_Dark_Mode_Shortcode;
			new WPDM\includes\WP_Dark_Mode_Hooks;

			require_once WPDM_BASE_PATH . 'includes/scss.inc.php';


			/** load gutenberg block */
			require_once WPDM_BASE_PATH .  'block/plugin.php';

			if (!is_admin()) {
				new WPDM\includes\WP_Dark_Mode_Theme_Supports;
			}

			//admin includes
			if (is_admin()) {
				new WPDM\includes\WP_Dark_Mode_Settings;
			}
		}
		...
		public function plugin_action_links($links) {
			$links[] = sprintf(
				'<a href="%1$s">%2$s</a>',
				admin_url('options-general.php?page=wp-dark-mode-settings'),
			...
			if (!is_plugin_active('wp-dark-mode-pro/plugin.php') && !is_plugin_active('wp-dark-mode-ultimate/plugin.php')) {
				$links[] = sprintf(
					...
					'https://wppool.dev/wp-dark-mode-pricing',
		...
		public function get_template($name = null, $args = false) {
			...
			if (!$template) {
				$template = WPDM_BASE_PATH . "templates/$name.php";
			}
		...
		public function register_widget() {
			...
			require_once WPDM_BASE_PATH . 'elementor/class-elementor-widget.php';
		...
		public function add_notice($class, $message, $only_admin = '') {
			...
			if (
				is_string($message) && is_string($class)
